"""
File: train_ppo.py
Description: PPO training script (parameters are fully aligned in config.py; can be used directly in continuous action scenarios)
Dependencies:
  - stable_baselines3
...
def main():
    ...
    # Allow train_all_overnight_v2.py to override total steps and seed via environment variables
    ...
    # PPO hyperparameter: read GUI first, TRAINING_CONFIG as supplementary coverage
    ...
    print("=" * 80)
    print(f"[PPO] This training version: {run_tag}")
    print(f"[PPO] Log directory: {LOG_DIR}")
    print(f"[PPO] Model version directory: {BEST_MODEL_DIR}")
    print(f"[PPO] TensorBoard: {TENSORBOARD_LOG_DIR}")
    print(f"[PPO] Training Steps: {TOTAL_TIMESTEPS:,}")
    print(f"[PPO] Two-stage training: {TWO_STAGE_TRAIN}")
    print("=" * 80)
    ...
    try:
        ...
        print(f"[PPO] Failed to create environment: {e}")
        traceback.print_exc()
        return

    # Model (under continuous action, PPO uses Gaussian strategy)
    model = PPO(
        policy="MlpPolicy",
        env=train_env,
        n_steps=n_steps,
    ...
    cost_curve_cb = CostCurveCallback(
        ...
        agent_name=agent_name_for_plot, # <--- (1) Add missing agent_name
        save_path=BEST_MODEL_DIR, # <--- (2) Correct the parameter name log_dir -> save_path
    ...
    try:
        ...
        print("\n[PPO] Interrupt, save model...")
    except Exception as e:
        print(f"[PPO] Training exception: {e}")
        traceback.print_exc()
    finally:
        try: cost_curve_cb.save_data()
        except Exception: pass
        print(f"[PPO] Save the final model to: {os.path.abspath(FINAL_MODEL_PATH)}")
        ...
        try:
            ...
            print(f"[PPO] Quick copy of the latest model: {os.path.abspath(LATEST_MODEL_PATH)}")
        except Exception as copy_err:
            print(f"[PPO] Failed to write the latest model shortcut copy: {copy_err}")
