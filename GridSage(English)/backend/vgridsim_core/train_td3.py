...
"""
File: train_td3.py
Description: TD3 training script (align the parameters of config.py)
Dependencies:
  - stable_baselines3
...
def make_env(use_two_stage: bool):
    """
    Construct a single environment instance:
    - All "time related" parameters come from the config.py/GUI settings file
    """
    #GUI priority, otherwise fallback to the default value of CORE_PARAMS
    gui_params = load_gui_settings()

    # Override key switches of CORE_PARAMS with GUI settings (can be extended as needed)
    params = {
        ...
        # Other keys you need in env can continue to be obtained from CORE_PARAMS
    ...
    # Create environment; the two-stage switch reads from TRAINING_CONFIG and also allows function parameter overrides
    env = PowerGridEnv(gui_params=params, use_two_stage_flow=use_two_stage)
    return env


def main():
    # ============== Unified path and configuration ==============
    ...
    # Whether to enable two phases during the training period (if it is too slow, you can turn it off in TRAINING_CONFIG, evaluate and turn it on again)
    TWO_STAGE_TRAIN = bool(TRAINING_CONFIG.get("two_stage_training", True))

    # Full read configuration of total training steps, frequency, etc. to ensure consistency with SAC
    ...
    # Log/save location (divided by Algorithm name, stage, Training Steps and timestamp)
    ...
    print("=" * 80)
    print(f"[TD3] This training version: {run_tag}")
    print(f"[TD3] Log directory: {LOG_DIR}")
    print(f"[TD3] Model version directory: {BEST_MODEL_DIR}")
    print(f"[TD3] TensorBoard: {TENSORBOARD_LOG_DIR}")
    print(f"[TD3] Training Steps: {TOTAL_TIMESTEPS:,}")
    print(f"[TD3] Two-stage training: {TWO_STAGE_TRAIN}")
    print("=" * 80)

    # ============== Construct training/evaluation environment ==============
    try:
        # Evaluate the environment
        eval_env_raw = make_env(use_two_stage=True) # Evaluation suggestions are always in two stages to ensure fairness and authenticity
        print(f"Evaluate the environment flow mode: {'two-stage' if eval_env_raw.use_two_stage_flow else 'single-stage'}")
        ...
        print(f"Training environment flow mode: {'two-stage' if train_env_raw.use_two_stage_flow else 'single-stage'}")
        train_env = wrap_training_env(
            train_env_raw, gamma=normalization_gamma, training_config=TRAINING_CONFIG
        )
        # SB3 accepts VecEnv, which is encapsulated in a single environment.
        ...
        print(f"[TD3] Failed to create environment: {e}")
        traceback.print_exc()
        return

    # ============== Construct TD3 model (including motion noise) ==============
    ...
    # Clipping range of target noise
    ...
    # Need to detect the action dimension first
    ...
    model = TD3(
        ...
        verbose=1,
        seed=RANDOM_SEED,
        # You can add other hyperparameters of TD3 here (such as learning rate, batch size, etc.), or they can be put into TRAINING_CONFIG extended reading
    )

    # ============== Callbacks: evaluation, checkpoint, cost curve ==============
    eval_cb = EvalCallback(
        eval_env=eval_env,
        best_model_save_path=BEST_MODEL_DIR,
        log_path=LOG_DIR,
        eval_freq=EVAL_FREQ,
        n_eval_episodes=3, # can be placed in TRAINING_CONFIG
        deterministic=True,
        render=False
    )
    ...
    try:
        ...
        print("\n[TD3] Received interrupt signal, save current model...")
    except Exception as e:
        print(f"[TD3] An exception occurred during training: {e}")
        traceback.print_exc()
    finally:
        ...
        try:
            ...
            pass
        print(f"[TD3] Save the final model to: {os.path.abspath(FINAL_MODEL_PATH)}")
        ...
        try:
            ...
            print(f"[TD3] Latest model shortcut copy: {os.path.abspath(LATEST_MODEL_PATH)}")
        except Exception as copy_err:
            print(f"[TD3] Failed to write the latest model shortcut copy: {copy_err}")
        try:
            model.logger.close()
        except Exception:
            pass
