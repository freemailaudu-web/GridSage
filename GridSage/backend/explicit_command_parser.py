...
def _number_near_keyword(text: str, keywords: List[str]) -> Optional[float]:
    for keyword in keywords:
        patterns = [
            rf"{keyword}.{{0,20}}?(-?\d+(?:\.\d+)?)\s*倍",
            rf"{keyword}.{{0,20}}?(-?\d+(?:\.\d+)?)\s*%",
            rf"{keyword}.{{0,20}}?(?:设置|设为|改成|调整为|提高到|降低到|set to|to)[^\d-]{{0,8}}(-?\d+(?:\.\d+)?)",
        ]
...
def _has_time_expression(text: str) -> bool:
    text = text or ""
    return bool(
        re.search(r"\d{1,2}\s*(?:点|时|:)?\s*(?:到|至|[-~－—])\s*\d{1,2}\s*(?:点|时)?", text)
        or any(word in text for word in ["中午", "午间", "傍晚", "晚间", "晚上", "夜间", "凌晨", "分时", "时序", "错配", "鸭子曲线"])
    )


def _hour_range(text: str, default_start: int, default_end: int) -> Tuple[int, int]:
    match = re.search(
        r"(\d{1,2})\s*(?:点|时|:)?\s*(?:到|至|[-~－—])\s*(\d{1,2})\s*(?:点|时)?",
        text or "",
    )
    if match:
...
def _default_directional_value(text: str, high_value: float, low_value: float, neutral: float = 1.0) -> Optional[float]:
    if re.search(r"(提高|升高|高|大发|集中|峰值|peak|increase)", text or "", re.IGNORECASE):
        return high_value
    if re.search(r"(降低|下降|调低|低|削减|decrease)", text or "", re.IGNORECASE):
        return low_value
    return neutral
...
def _detect_time_profile(text: str) -> Optional[Dict[str, Any]]:
    ...
    if "源荷" in text or "错配" in text:
        return {
            "profile_name": "custom_time_profile",
            "profile_description": "源荷时序错配：中午光伏高，傍晚负荷和 EV 充电高。",
    ...
    profile: Dict[str, Any] = {
        "profile_name": "custom_time_profile",
        "profile_description": "由自然语言解析生成的分时倍率。",
    }
    has_component = False

    if _phrase_has_any(text, ["光伏", "pv", "solar"]):
        start, end = _hour_range(text, 10, 14)
        pv_value = _number_near_keyword(text, ["光伏", "PV", "pv", "solar"])
        if pv_value is None:
    ...
    if _phrase_has_any(text, ["负荷", "load"]):
        default_range = (0, 6) if any(word in text for word in ["夜间", "凌晨"]) else (18, 22)
        start, end = _hour_range(text, *default_range)
        load_value = _number_near_keyword(text, ["负荷", "load", "Load"])
    ...
    if _phrase_has_any(text, ["EV", "ev", "电动车", "充电"]):
        start, end = _hour_range(text, 18, 22)
        ev_value = _number_near_keyword(text, ["EV", "ev", "电动车", "充电"])
        if ev_value is None:
...
def _append_explicit_node_commands(text: str, commands: List[Dict[str, Any]]) -> None:
    ...
    delete_request = bool(re.search(r"(删除|删掉|移除|去掉|停用|禁用|关闭|remove|delete|disable|turn off)", text, re.IGNORECASE))
    if delete_request:
        device_type = None
        if "发电机" in text or "generator" in lower or re.search(r"\bgen\b", lower):
            device_type = "generator"
        elif "光伏" in text or "pv" in lower or "solar" in lower:
            device_type = "pv"
        elif "风电" in text or "wind" in lower:
            device_type = "wind"
        elif "储能" in text or "ess" in lower or "battery" in lower:
            device_type = "ess"
        elif "充电站" in text or "ev" in lower or "充电" in text:
    ...
    payload = {}
    if "pv" in lower or "光伏" in text:
        value = _number_before_units(text, r"kw|kW|千瓦")
        payload["add_pv_kw"] = value if value is not None else 300.0
    if "wind" in lower or "风电" in text:
        value = _number_before_units(text, r"kw|kW|千瓦")
        payload["add_wind_kw"] = value if value is not None else 300.0
    if "ess" in lower or "储能" in text or "battery" in lower:
        value = _number_before_units(text, r"kwh|千瓦时")
        power_value = _number_before_units(text, r"kw(?!h)|千瓦(?!时)")
    ...
    if "ev" in lower or "充电" in text:
        value = _number_before_units(text, r"spots?|chargers?|个|個|台|座|桩|樁|车位|車位")
        payload["add_ev_spots"] = int(value) if value is not None else 5
    if "负荷" in text or "load" in lower:
        value = _number_before_units(text, r"kw|kW|千瓦")
...
def _explicit_algo(text: str, commands: List[Dict[str, Any]]) -> Optional[str]:
    ...
    if "baseline" in lower or "基线" in text:
        return "Baseline"
...
def _is_evaluation_request(text: str) -> bool:
    return bool(
        re.search(
            r"(评估|评价|测试|对比|比较|evaluate|evaluation|test|compare|comparison)",
            text or "",
            re.IGNORECASE,
        )
    )
...
def _is_trained_model_reference(text: str) -> bool:
    return bool(
        re.search(
            r"(训练好|训练好的|训练完成|已训练|刚刚训练|训练后|trained model|trained)",
            text or "",
            re.IGNORECASE,
        )
    )
...
def _extract_steps(text: str) -> Optional[int]:
    patterns = [
        r"(?<![A-Za-z0-9])(\d{1,9})\s*(?:步|steps?|step|timesteps?)",
        r"(?:训练|再训练|重新训练|train|retrain|training)\D{0,40}?(?<![A-Za-z])(\d{1,9})(?![A-Za-z])",
    ]
    for pattern in patterns:
...
def _model_reference_steps(text: str) -> Optional[int]:
    if not _is_evaluation_request(text):
        return None
    if not re.search(r"(模型|model|checkpoint|权重)", text or "", re.IGNORECASE):
        return None
...
def _requested_comparison_targets(text: str, commands: List[Dict[str, Any]]) -> List[str]:
    text = text or ""
    has_eval = bool(re.search(r"(评估|评价|测试|对比|比较|横向|evaluate|evaluation|compare|comparison)", text, re.IGNORECASE))
    targets: List[str] = []
    token_pattern = r"baseline|基线|SAC|PPO|TD3|DDPG"
    for match in re.finditer(token_pattern, text, re.IGNORECASE):
        token = match.group(0)
        if token.lower() == "baseline" or token == "基线":
...
def merge_explicit_global_commands(user_input: str, commands: List[Any]) -> List[Dict[str, Any]]:
    ...
    if time_profile:
        ...
        if "场景" in text or "鸭子曲线" in text or "duck curve" in text.lower():
            scenario_name = "鸭子曲线时序场景" if time_profile.get("profile_name") == "duck_curve" else "分时时序场景"
            _upsert_command(merged, "scenario_name", scenario_name)
            _upsert_command(merged, "scenario_description", time_profile.get("profile_description", "分时负荷、光伏和 EV 倍率场景。"))
