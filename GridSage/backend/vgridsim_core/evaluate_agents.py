...
def _add_voltage_metrics(metrics, voltages_data):
    ...
    if not values:
        metrics.setdefault("最低节点电压(pu)", np.nan)
        metrics.setdefault("最高节点电压(pu)", np.nan)
        metrics.setdefault("最大电压偏差(pu)", np.nan)
        return
    metrics["最低节点电压(pu)"] = min(values)
    metrics["最高节点电压(pu)"] = max(values)
    metrics["最大电压偏差(pu)"] = max(abs(v - 1.0) for v in values)
    metrics["电压极差(pu)"] = max(values) - min(values)
...
def _add_pv_curtailment_metrics(metrics, grid, pvw_dispatch_data, gui_params, pvw_available_data=None):
    ...
    if not pvw_devices:
        metrics["光伏风电消纳率(%)"] = 100.0
        metrics["弃光弃风量(kWh)"] = 0.0
        return
    ...
    metrics["弃光弃风量(kWh)"] = renewable_curtail
    ...
    for line_id, series in (line_powers_data or {}).items():
        for value in _iter_numeric_values(series):
            ...
            if flow > max_flow:
                max_flow = flow
                max_line_id = line_id
    metrics["最大线路功率流(pu)"] = max_flow
    if max_line_id:
        metrics["最大线路功率流线路"] = max_line_id
...
def run_baseline_stage_two(grid: object, baseline_data: object, gui_params: object) -> object:
    ...
    if AUDIT_TIMESTEP is not None and AUDIT_TIMESTEP < time_steps:
        ...
        for sop_id, flow_data in sop_flows_data.items():
            if sop_id in sop_gen_map:
                ...
                gen2._p, gen2._q = p_transfer - loss_transfer, q2_injection  # 注入端功率要减去损耗
    ...
    if baseline_data.get("solver_note"):
        metrics["求解说明"] = baseline_data["solver_note"]
    metrics["总成本"] = sum(
        m for k, m in metrics.items()
        if '网损' not in k and isinstance(m, (int, float))
    )
    metrics["功率平衡松弛惩罚"] = baseline_data.get('slack_penalty', 0)
    metrics["EV未充满惩罚"] = baseline_data.get('ev_not_full_penalty', 0)
    metrics["SOP容量松弛惩罚"] = baseline_data.get('sop_slack_penalty', 0)
    metrics["NOP电压松弛惩罚"] = baseline_data.get('nop_slack_penalty', 0)
    metrics["总惩罚成本"] = (
        metrics["功率平衡松弛惩罚"]
        + metrics["EV未充满惩罚"]
        + metrics["SOP容量松弛惩罚"]
...
def evaluate_rl_agent(model, env, seed, obs_normalizer=None):
    ...
    while not terminated:
        ...
        metrics["电压惩罚成本"] += max(0.0, -float(info.get("voltage_penalty_unscaled", 0.0) or 0.0))
        metrics["OpenDSS失败惩罚成本"] += max(
            0.0,
        ...
        metrics["EV未满足惩罚成本"] += ev_shortage_cost
        metrics["EV未充满惩罚"] += ev_shortage_cost
        
        urgency_penalty = float(info.get("ev_urgency_penalty", 0.0) or 0.0)
        metrics["ev_urgency_penalty"] += urgency_penalty
        metrics["EV紧迫度惩罚成本"] += max(0.0, -urgency_penalty)
        metrics["功率平衡松弛惩罚"] += float(info.get("slack_penalty", 0.0) or 0.0)
        metrics["SOP容量松弛惩罚"] += float(info.get("sop_slack_penalty", 0.0) or 0.0)
        metrics["NOP电压松弛惩罚"] += float(info.get("nop_slack_penalty", 0.0) or 0.0)
    ...
    metrics["总惩罚成本"] = sum(
        metrics[k] for k in [
            "功率平衡松弛惩罚", "SOP容量松弛惩罚", "NOP电压松弛惩罚",
            "EV未充满惩罚", "EV紧迫度惩罚成本", "电压惩罚成本", "OpenDSS失败惩罚成本",
        ]
    )
    if reward_mode == "station_operator":
...
def evaluate_baseline(gui_params, seed, stations_list, grid, use_two_stage=False):
    ...
    if use_two_stage:
        ...
        metrics["重构方案ID"] = reconf.get("plan_id", "R0")
        metrics["闭合NOP"] = reconf.get("close_nop_id") or "无"
        metrics["打开支路"] = reconf.get("open_line_id") or "无"
        metrics["拓扑径向"] = bool(reconf.get("is_radial", True))
        metrics["拓扑连通"] = bool(reconf.get("is_connected", True))
        ...
        if stage_one_data.get("solver_note"):
            metrics["求解说明"] = stage_one_data["solver_note"]
        metrics["总成本"] = sum(v for v in metrics.values() if isinstance(v, (int, float)))
        metrics["功率平衡松弛惩罚"] = stage_one_data.get('slack_penalty', 0)
        metrics["EV未充满惩罚"] = stage_one_data.get('ev_not_full_penalty', 0)
        metrics["SOP容量松弛惩罚"] = stage_one_data.get('sop_slack_penalty', 0)
        metrics["NOP电压松弛惩罚"] = stage_one_data.get('nop_slack_penalty', 0)
        metrics["总惩罚成本"] = (
            metrics["功率平衡松弛惩罚"]
            + metrics["EV未充满惩罚"]
            + metrics["SOP容量松弛惩罚"]
