...
def main():
    ...
    # 每次训练都保存到独立版本目录，避免 1000 步模型被后续 500 步训练覆盖。
    # 目录名同时包含算法、阶段、步数、seed、时间戳，便于评估端按“X步模型”精确匹配。
    ...
    try:
        ...
        # manifest 让后端评估端和人工排查都能知道这个目录对应多少训练步。
        ...
        # 保留“最新模型”的兼容入口，但不要用它来表示某个具体步数版本。
        try:
            shutil.copy2(FINAL_MODEL_PATH, LATEST_MODEL_PATH)
            print(f"[SAC] 最新模型快捷副本: {os.path.abspath(LATEST_MODEL_PATH)}")
        except Exception as copy_err:
            print(f"[SAC] 写入最新模型快捷副本失败: {copy_err}")
        print("[SAC] 训练结束。")
        try:
            import sys
