...
def main():
    ...
    print("=" * 80)
    print(f"[DDPG] 本次训练版本: {run_tag}")
    print(f"[DDPG] 日志目录: {LOG_DIR}")
    print(f"[DDPG] 模型版本目录: {BEST_MODEL_DIR}")
    ...
    try:
        ...
        print(f"[DDPG] 保存最终模型到: {os.path.abspath(FINAL_MODEL_PATH)}")
        ...
        try:
            ...
            print(f"[DDPG] 最新模型快捷副本: {os.path.abspath(LATEST_MODEL_PATH)}")
        except Exception as copy_err:
            print(f"[DDPG] 写入最新模型快捷副本失败: {copy_err}")
